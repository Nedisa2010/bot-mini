...
// 🔧 MODIFIED pair.js BASE FILE // Note: This is a template with the requested features added: // ✅ Owner React Command // ✅ Owner-only Channel Following Command // ✅ Owner-only Channel React Command // USE THIS AS YOUR UPDATED pair.js — integrate into your bot

/* ----------------------------------------- OWNER SETTINGS -------------------------------------------- */ const OWNER = "94761207139"; // << CHANGE IF NEEDED

/* ----------------------------------------- ADD INTO YOUR COMMAND HANDLER (search: switch(command)) -------------------------------------------- */

// OWNER REACT COMMAND case 'oreact': { if (senderNumber !== OWNER) return socket.sendMessage(sender, { text: "❌ Only Owner Can Use This Command" })

const quotedMsg = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
const reaction = args[0] || "❤️";

if (!quotedMsg) return socket.sendMessage(sender, { text: "💬 Reply to a message to react" });

await socket.sendMessage(
    sender,
    { react: { text: reaction, key: msg.message.extendedTextMessage.contextInfo.stanzaId } }
);
...
// OWNER-ONLY CHANNEL FOLLOW case 'ocf': { if (senderNumber !== OWNER) return socket.sendMessage(sender, { text: "❌ Only Owner can use this command!" });

if (!args[0]) return socket.sendMessage(sender, { text: "📌 Usage: .ocf <channel_jid>" });

const jid = args[0];
if (!jid.endsWith("@newsletter")) return socket.sendMessage(sender, { text: "❌ Invalid Channel JID" });

try {
    await socket.newsletterFollow(jid);
    await socket.sendMessage(sender, { text: `✅ Owner followed channel:

${jid}}); } catch (e) { await socket.sendMessage(sender, { text:❌ Error: ${e.message}` }); }

break;

}

// OWNER CHANNEL REACT case 'ocr': { if (senderNumber !== OWNER) return socket.sendMessage(sender, { text: "❌ Only Owner Allowed" });

const target = args[0];
const emoji = args[1] || "❤️";

if (!target) return socket.sendMessage(sender, { text: "📌 Usage: .ocr <message_id> <emoji>" });

try {
    await socket.newsletterReactMessage(config.NEWSLETTER_JID, target, emoji);
    await socket.sendMessage(sender, { text: `❤️ Reacted to message ID ${target}` });
} catch (e) {
...
/* ----------------------------------------- END OF MODIFICATIONS — MERGE INTO YOUR FILE -------------------------------------------- */
